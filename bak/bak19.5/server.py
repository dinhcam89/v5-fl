...
class EnhancedFedAvgWithGA(fl.server.strategy.FedAvg):
    ...
    def configure_fit(self, server_round, parameters, client_manager):
        ...
        config = {
            ...
            "ipfs_hash": ""  # Initialize with empty string
        }
        ...
        if server_round == 1:
            ...
            # Use empty parameters for round 1 since clients initialize models
            params_to_send = ndarrays_to_parameters([np.array([])])
        else:
            # ROUNDS 2+: Fine-tuning configuration
            config.update({
                "ga_stacking": True,  # Still use GA-Stacking but with existing models
                "local_epochs": max(2, 6 - server_round//2),  # Fewer epochs for fine-tuning
                ...
                "fine_tuning_params": {
                    ...
                    "reduced_generations": max(5, min(10, 20 - server_round))  # Reduce GA generations
                }
            })
            
            # Include global IPFS hash if available (for clients that might still use IPFS)
            ...
            # For rounds 2+, send the actual parameters instead of empty ones
            params_to_send = parameters
        
        fit_ins = FitIns(params_to_send, config)
    ...
    def aggregate_fit(
        ...
        if any_ensemble:
            ...
            logger.info(f"Using Model Transfer Mode: {'IPFS' if using_ipfs else 'Direct Parameter Exchange'}")
