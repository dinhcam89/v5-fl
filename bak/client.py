...
class GAStackingClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters: Parameters, config: Dict[str, Scalar]) -> Tuple[List[np.ndarray], int, Dict[str, Scalar]]:
        ...
        if round_num == 1:
            ...
            logger.info(f"Round {round_num}: Fine-tuning existing base models (IPFS-only, ignoring parameters)")
            try:
                ...
                # Fine-tune the models (do NOT use server weights)
...
def start_client(
    ...
    fl.client.start_client(
        ...
        reconnect=True  # Add this parameter
    ) 
